...
(** val finish_print :
    'a1 coq_PrettyPrinter -> ('a1 * String.t, String.t) result **)

let finish_print pp =
  bind (Obj.magic coq_Monad_result) (Obj.magic finish_print_lines pp)
    (fun x ->
    let (a, lines) = x in
    ret (Obj.magic coq_Monad_result) (a, (String.concat nl lines)))

(** val get_indent : nat coq_PrettyPrinter **)

let get_indent pps =
  Ok ((hd O pps.indent_stack), pps)
...
(** val pop_use : unit coq_PrettyPrinter **)
