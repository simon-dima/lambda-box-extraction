...
| String.String (sc, s0) ->
  ...
  then (match with_str with
        | String.EmptyString -> Some (cont s0)
        | String.String (wsc, ws) -> starts_with_cont wsc ws cont s0)
  else None
...
let replace orig new0 =
  match orig with
  | String.EmptyString -> (fun s -> s)
  | String.String (origc, origs) ->
    let rec replace0 s =
      match starts_with_cont origc origs replace0 s with
      | Some s0 -> String.append new0 s0
      | None ->
        (match s with
         | String.EmptyString -> String.EmptyString
         | String.String (c, s0) -> String.String (c, (replace0 s0)))
    in replace0
